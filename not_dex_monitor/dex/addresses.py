...
# Pool addresses verified live via Balancer v3 API (api-v3.balancer.fi/graphql)
# -- only pools with swapEnabled=true AND non-trivial TVL are kept here.
# Pairs with no liquid Balancer pool today (WETH/USDT, USDC/DAI, USDT/DAI,
# WBTC/USDT, WBTC/DAI) are intentionally omitted -- the adapter's supports_pair
# will return False for them and the worker will skip Balancer for those pairs.
BALANCER_POOL_ADDRESSES: Dict[Tuple[str, str], List[str]] = {
  # 50WBTC-50WETH weighted pool (~$1.8M TVL)
  ("WBTC", "WETH"): [
    "0xa6f548df93de924d73be7d25dc02554c6bd66db5",
  ],
  # 80WETH-20USDC weighted pool (~$185k TVL)
  ("WETH", "USDC"): [
    "0x96646936b91d6b9d7d0c47c496afbf3d6ec7b6f8",
  ],
  # 60WETH-40DAI weighted pool (~$167k TVL)
  ("WETH", "DAI"): [
    "0x0b09dea16768f0799065c475be02919503cb2a35",
  ],
  # GHO/USDT/USDC composable stable pool (~$120k TVL) -- replaces deprecated staBAL3.
  # DAI is NOT in this pool; do not use it for USDC/DAI or USDT/DAI.
  # Same pool also covers GHO swaps with USDC and USDT.
  ("USDC", "USDT"): [
...
# Pool addresses verified live via GeckoTerminal API for the DODO PMM
# Ethereum DEX. Pairs with WETH/WBTC have no active DODO pools today --
# liquidity is concentrated in stable->stable PMM pools.
DODO_V2_POOL_ADDRESSES: Dict[Tuple[str, str], List[str]] = {
  # USDT/USDC PMM pool -- ~$308k TVL, ~$54M 24h volume (most active DODO pair).
  ("USDC", "USDT"): [
    "0x04571c32a4e1c5f39bc3a238cb95b215058c432c",
    "0xc9f93163c99695c6526b799ebca2207fdf7d61ad", # secondary
  ],
  # DAI/USDT PMM pool -- smaller but live.
  ("DAI", "USDT"): [
